...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT nomeprod, descricao, valor, imagem FROM produtos WHERE id = $id";
...
if (isset($_POST['editar_registro'])) {
    ...
    if ($_FILES['nova_imagem']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!empty($imagemNome) && file_exists("C:/wamp642/www/4IB/" . $imagemNome)) {
            unlink("C:/wamp642/www/4IB/" . $imagemNome);
        }
    ...
    $sql = "UPDATE produtos SET nomeprod = '$nomeprod', descricao = '$descricao', valor = '$valor', imagem = '$imagemNome' WHERE id = $id";
...
<body>
    ...
    <form method="POST" action="editar.php?id=<?php echo $id; ?>" enctype="multipart/form-data">
        ...
        <?php
        if (!empty($imagemNome) && file_exists("C:/wamp642/www/4IB/" . $imagemNome)) {
            echo '<h2>IMAGEM ATUAL</h2><img src="' . $imagemNome . '" alt="Imagem Atual"><br>';
        } else {
